...
void Tile::SetTile(const char* Name, int X, int Y, int Width, int Height)
{
	_NameTile = Name;
...
AnimationSprite::AnimationSprite(LPDIRECT3DDEVICE9 d3ddv)
{
	_Texture = nullptr;
...
bool AnimationSprite::ReadXML(const char* XMLPath)
{
...
bool AnimationSprite::Load(const char* ImagePath, const char* XMLPath, D3DCOLOR Transcolor)
{
...
void AnimationSprite::Render(float DeltaTime, float X, float Y, float ScaleSize, float AnimationRate, float FlipX)
{
	D3DXMATRIX Combined;
